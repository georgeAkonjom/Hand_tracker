...
from detectors.hand.boolean_gesture_detector import BooleanGestureDetector
...
class HandDetectorWrapper(BaseDetector):
    def __init__(self, model_path='models/hand_landmarker.task'):
        ...
        self.bool_detector = BooleanGestureDetector()
    ...
    def get_latest_data(self):
        ...
        for idx, hand_landmarks in enumerate(self.latest_result.hand_landmarks):
            ...
            gesture_name, _, facing = self.bool_detector.detect_gesture(hand_landmarks, handedness)
    ...
    def draw(self, frame):
        ...
        for idx, hand in enumerate(hands_data):
            ...
            cv2.putText(frame, f"{hand['handedness']} {hand['facing']}: x={wrist['x']:.2f}, y={wrist['y']:.2f}", 
                        (20, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
            cv2.putText(frame, f"Gesture: {hand['gesture']}", 
